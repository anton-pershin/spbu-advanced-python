...
python -m p_02_classes_state_behavior.template.check

The checks intentionally use only the public API:

from p_02_classes_state_behavior.template.lrucache import LRUCache
"""
...
def test_eviction_order() -> None:
    cache = LRUCache(capacity=2)
    cache.put("a", 1)  # MRU: a
    cache.put("b", 2)  # MRU: b, then a

    # touch 'a' -> MRU becomes: a, then b
    assert cache.get("a") == 1

    # insert 'c' -> evict LRU ('b')
    cache.put("c", 3)
...
def test_update_existing_key() -> None:
    ...
    cache.put("a", 10)  # update + move to MRU
...
def test_capacity_five_eviction() -> None:
    ...
    # touch some keys to change recency
    assert cache.get("b") == 2
    assert cache.get("a") == 1

    # insert one more -> evict the current LRU ('c')
    cache.put("f", 6)
